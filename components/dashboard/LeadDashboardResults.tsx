...
export function LeadDashboardResults({ rows, portfolioMode }: { rows: LeadDashboardRow[]; portfolioMode: boolean }) {
  if (!rows.length) return <Card><CardContent className="py-14 text-center"><p className="text-base font-medium">No Leads match these filters</p><p className="mt-2 text-sm text-muted-foreground">Clear the filters or import a business-finder CSV to add Leads.</p></CardContent></Card>
...
function DesktopRow({ row, portfolioMode }: { row: LeadDashboardRow; portfolioMode: boolean }) {
  ...
  return <tr className="transition-colors hover:bg-muted/35">
...
function MobileCard({ row, portfolioMode }: { row: LeadDashboardRow; portfolioMode: boolean }) {
  const availability = demoAvailability(row, portfolioMode)
  return <Card className="transition-shadow hover:shadow-md"><CardContent className="space-y-4 p-5"><div className="flex items-start justify-between gap-4"><Business lead={row.lead} /><ReviewLink lead={row.lead} /></div><div className="flex flex-wrap gap-2"><Badge variant={statusBadgeVariant(row.lead.pipeline_status)}>{formatDashboardStatus(row.lead.pipeline_status)}</Badge><Badge variant={statusBadgeVariant(row.lead.sales_status)}>Sales: {formatDashboardStatus(row.lead.sales_status)}</Badge>{row.lead.demo_creation_approved_at ? <Badge variant="outline">Demo creation approved</Badge> : null}</div><WorkflowSummary run={row.workflowRun} />{row.demoSite ? <div className="flex flex-col gap-1 border-t pt-3 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-muted-foreground">{availability}</span><Link className="inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/dashboard/review/${row.lead.id}/demo-preview`}><Eye className="h-4 w-4" /> Admin preview</Link></div> : null}</CardContent></Card>
}
