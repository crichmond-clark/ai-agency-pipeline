...
type ActionConfig = { key: string; label: string; endpoint: string; enabled: boolean; disabledReason?: string }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-2 sm:grid-cols-2">{actions.map((action) => { const isBusy = busyAction === action.key; const isDestructive = action.key === 'reject'; return <div className="flex items-center justify-between gap-3 rounded-lg border p-3 transition-colors hover:bg-muted/40" key={action.key}><div className="min-w-0"><p className="truncate text-sm font-medium">{action.label}</p>{!action.enabled && action.disabledReason ? <p className="mt-1 text-xs text-muted-foreground">{action.disabledReason}</p> : null}</div><Button aria-label={action.label} className="shrink-0" disabled={!action.enabled || Boolean(busyAction)} onClick={() => runAction(action)} size="sm" type="button" variant={isDestructive ? 'outline' : action.enabled ? 'default' : 'secondary'}>{isBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : action.enabled ? <Play className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}{isBusy ? 'Running…' : action.enabled ? 'Run' : 'Done'}</Button></div> })}</div>
