...
export async function POST(request: Request, { params }: { params: Promise<{ demoSiteId: string }> }) {
  ...
  try {
    ...
    const updatedDemoSite = await payload.update({
      ...
      id: demoSite.id,
      data: { qa_report: { ...(typeof demoSite.qa_report === 'object' && demoSite.qa_report ? demoSite.qa_report : {}), screenshots } },
    })

    await recordWorkflowRun(payload, { operation: 'screenshot_capture', status: 'succeeded', lead: typeof demoSite.lead === 'object' ? demoSite.lead.id : demoSite.lead, demo_site: demoSite.id, started_at: startedAt, summary: 'Captured desktop and mobile screenshots' })
    return Response.json({ demo_site: updatedDemoSite, screenshots })
  } catch (error) {
